...
u8 Te4[256] __attribute__ ((aligned(64)))  = {
...
u8 Td4[256] __attribute__ ((aligned(64)))= {
...
{    
    ...
    //print_aes_access(s0, s1, s2, s3, 0);
    return check_aes_access(s0, s1, s2, s3, 0);
    return only_one_access(s0, s1, s2, s3, bytes, 0);
    ...
    print_aes_access(s0, s1, s2, s3, 1);
    ...
    for (rk+=8,r=key->rounds-2; r>0; rk+=4,r--) {
        ...
        s0 = t[0]; s1 = t[1]; s2 = t[2]; s3 = t[3];
	print_aes_access(s0, s1, s2, s3, 10 - r);
...
int AES_decrypt2(const unsigned char *in, unsigned char *out, const AES_KEY *key, int bytes)
{
    const u32 *rk;
    u32 s0, s1, s2, s3, t[4];
    int r;
                 ...
                 const AES_KEY *key, int bytes)
{
    ...
    if ( !check_aes_access(s0, s1, s2, s3, 0) ) {
        //printf("xxx\n");
        return 0;
    }
